...
export function AddOrEditBookForm({ bookId, initial }: AddBookFormProps) {
  const router = useRouter();
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState("");
  const [form, setForm] = useState({
    ...
    totalCopies: String(initial?.totalCopies ?? 1),
  });
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      const res = await fetch(bookId ? `/api/school/library/${bookId}` : "/api/school/library", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ ...form, totalCopies: parseInt(form.totalCopies, 10) || 1 }),
      });
